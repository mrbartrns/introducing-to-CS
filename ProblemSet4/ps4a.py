...
def isValidWord(word, hand, wordList):
    ...
    if word not in wordList:
        ...
        for c in word:
            if c not in newHand or newHand[c] == 0:
                return False
            else:
                newHand[c] -= 1
    return True
    ...
    while flag:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                print('"{word}" earned {score} points. Total: {total} points'.format(word = word, score = getWordScore(word, n), total = total))
                # Update the hand
